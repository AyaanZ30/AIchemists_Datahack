        ...
        "contents": [{
            "parts": [{
                "text": f"{improvement_prompt}{recommendations_prompt}Generate 5 multiple-choice questions on the topic '{topic}' for difficulty level: '{difficulty_level} on a scale of 1-10' strictly. Make sure the questions are unique, brainstorming and technical. "
                        "Higher difficulty level means generate more difficult challenging and technical questions. Make sure to test user's understanding of the topic."
...
def read_text_from_image(image_path):
    ...
    return extracted_text.strip()  # Return extracted text without extra spaces
...
if __name__ == "__main__":
    ...
    # Extract topic from the image
    topic = read_text_from_image(image_path)
    print(f"\nExtracted Topic from Image: '{topic}'")

    # Generate the questions based on the extracted topic
